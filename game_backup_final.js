...
const PLANE_CONFIGS = [
  ...
  { id: "legendary", name: "雷霆战机", icon: "🌩", hp: 200, damage: 30, speed: 7, cost: 3000, description: "传说战机，无敌之力" }
]
...
const LEVEL_CONFIGS = [
  ...
  { level: 5, name: "最终决战", waves: 7, enemies: ["elite"], boss: "boss3" }
]

const ACHIEVEMENT_CONFIGS = [
  { id: "first_game", name: "初次升空", icon: "🎮", description: "完成第一次游戏", reward: 100, condition: (s) => s.totalGames >= 1 },
  { id: "first_win", name: "初战告捷", icon: "🏆", description: "通关第一关", reward: 200, condition: (s) => s.wins >= 1 },
  { id: "score_1000", name: "千分达人", icon: "⭐", description: "单局达到1000分", reward: 300, condition: (s) => s.highScore >= 1000 },
  { id: "score_5000", name: "银翼飞将", icon: "🥈", description: "单局达到5000分", reward: 500, condition: (s) => s.highScore >= 5000 },
  { id: "kill_100", name: "百杀达人", icon: "💀", description: "累计击杀100敌机", reward: 200, condition: (s) => s.totalKills >= 100 },
  ...
  { id: "unlock_all", name: "战机收藏家", icon: "🏪", description: "解锁所有战机", reward: 1000, condition: (s) => s.unlockedPlanes.length >= PLANE_CONFIGS.length },
]
  ...
  bossShoot(playerPos) {
    ...
    switch (this.currentPhase) {
      ...
      case 2:
        ...
        break;
      case 3:
        const angle3 = Math.atan2(playerPos.y - this.position.y, playerPos.x - this.position.x);
        bullets.push(new Bullet(
          this.position.x,
          this.position.y + this.size / 2,
            Math.cos(angle3) * CONFIG.ENEMY_BULLET_SPEED * 1.5,
            Math.sin(angle3) * CONFIG.ENEMY_BULLET_SPEED * 1.5,
          this.damage * 2,
          '#ff0000'
        ));
        for (let i = 0; i < 12; i++) {
